...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-4 left-0 right-0 z-50">
      {/* Scroll progress bar */}
      <motion.div style={{ scaleX }} className="fixed top-0 left-0 right-0 h-[3px] origin-left bg-green-500 z-50" />
      ...
      <nav className="container mx-auto bg-black/60 backdrop-blur-md border border-green-900/40 rounded-2xl mx-4 p-4 flex items-center justify-between shadow-lg shadow-green-900/30">
        <div className="flex items-center gap-3">
          <span className="text-green-400 font-bold text-lg select-none">Destiny Uyi</span>
        </div>
        ...
        <div className="md:hidden">
          <button onClick={() => setOpen(v => !v)} className="p-2 rounded-md bg-green-900/40">
      ...
      {open && (
        <div className="md:hidden container mx-auto mx-4 mt-2 bg-black/70 backdrop-blur-md rounded-2xl p-4 border border-green-900/40 shadow-lg">
          <div className="flex flex-col gap-3">
            ...
            <a href="https://wa.me/2349038692680" className="py-2 text-left text-green-300">Hire Me</a>
          </div>
        </div>
      )}
    </header>
